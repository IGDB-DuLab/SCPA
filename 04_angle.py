...
import pandas as pd
import numpy as np
### input the folder path of raw data of control or RNAi or RNAi_add
File_Directory = r'Z:\xiaolong\project-summary\raw zip\Raw_data_RNAi'   

### input the corresponding folder of relative cell position distance matrix from "03_extract_distance_matrix.py"
Spatial_Distance_Matrix_Folder=r'Z:\xiaolong\work project\final\RNAi_distMatrix' 

### assign the output folder path and file name
Output= r'C:\Users\dell\Desktop\RNAi_division_angles_A-P.txt'  
...
def angle(ba,dc): #Calculate the angle between the space vectors ba and dc 
    Lba = np.sqrt((ba).dot(ba))
    Ldc = np.sqrt((dc).dot(dc))
    cos_angle = (ba).dot(dc)/(Lba*Ldc)
    angle = np.arccos(cos_angle)
    angle2=angle*360/2/np.pi
    return([angle,angle2])
...
for each_emb in raw_data_list:
    ...
    emb_output_temp=emb_output_temp[['X','Y']]
    ...
    #coordinate1[2]=0###Z to 0
    #coordinate2[2]=0

    #axis_coordinate=[coordinate1,coordinate2]
    #all_axis_coordinate.append(axis_coordinate)
    coordinate1=coordinate1[0:2]
    coordinate2=coordinate2[0:2]

    ba=coordinate2-coordinate1
    ba_vertical=np.array([ba[1],-ba[0]])
###################################    
    
    #emb_output_distance_selected_2=emb_output_info.loc[['EMS','ABp']]               
    #cell_name_index_2=list(zip(emb_output_distance_selected_2.index,emb_output_distance_selected_2['time']))
    #emb_output_temp_2=emb_output_distance_selected_2.drop(['time'],axis=1)
    #emb_output_temp_2.index=cell_name_index_2
    #emb_output_temp_2=emb_output_temp_2[['X','Y']]
    
    #cell_pair_distance_all_tp_2=ss.distance.pdist(emb_output_temp_2)

    #cell_pair_distance_2=pd.DataFrame(ss.distance.squareform(cell_pair_distance_all_tp_2),index=cell_name_index_2,columns=cell_name_index_2)
    #cell_pair_distance_array_2=np.array(cell_pair_distance_2)
    #cell_pair_timepoint_2=np.where(cell_pair_distance_array_2==np.max(cell_pair_distance_array_2))[0]####min distance or Fixed time point
    
    #cell_time1_2=cell_pair_distance_2.columns[cell_pair_timepoint_2[0]]
    #cell_time2_2=cell_pair_distance_2.index[cell_pair_timepoint_2[-1]]
    
    #time1_2=cell_time1_2[1]
   # time2_2=cell_time2_2[1]
    
    #if type(emb_output_info.loc['EMS'])==pd.core.series.Series:
        #EMS_coordinate=np.array(emb_output_info.loc['EMS'].drop('time')) 
        
    #elif type(emb_output_info.loc['EMS'])==pd.core.frame.DataFrame:
       # EMS_coordinate=np.array(emb_output_info.loc['EMS'][emb_output_info.loc['EMS']['time']==time1_2][['X','Y','Z']].iloc[0])  
    
   # if type(emb_output_info.loc['ABp'])==pd.core.series.Series:
        #ABp_coordinate=np.array(emb_output_info.loc['ABp'].drop('time')) 
    
    #elif type(emb_output_info.loc['ABp'])==pd.core.frame.DataFrame:
    
        #ABp_coordinate=np.array(emb_output_info.loc['ABp'][emb_output_info.loc['ABp']['time']==time2_2][['X','Y','Z']].iloc[0])      
    if type(emb_output_info.loc['EMS'])==pd.core.series.Series:
        EMS_coordinate=np.array(emb_output_info.loc['EMS'].drop('time')) 
        
    elif type(emb_output_info.loc['EMS'])==pd.core.frame.DataFrame:
        EMS_coordinate=np.array(emb_output_info.loc['EMS'].iloc[-1][['X','Y','Z']])  
    ...
    if type(emb_output_info.loc['ABp'])==pd.core.series.Series:
        ...
        ABp_coordinate=np.array(emb_output_info.loc['ABp'].iloc[-1][['X','Y','Z']])   
    EMS_coordinate=EMS_coordinate[0:2]
    ABp_coordinate=ABp_coordinate[0:2]    
    
    EMS_ABp=EMS_coordinate-ABp_coordinate
...
###################################################################    
    ...
    for cell in middle_cell_cell_list:
        #cell='Epl'
        mother=cell_name_transfer(cell)
        children_0=mother+'0'
        children_1=mother+'1'
        children_0=cell_name_transfer(children_0)
        children_1=cell_name_transfer(children_1)
        ...
        children_0_coordinate=np.array(children_0_position)[0:2]#
        children_1_coordinate=np.array(children_1_position)[0:2]
        
        dc=children_1_coordinate-children_0_coordinate
        
        angle_value=angle(ba,dc)[1]
        angle_value_2=angle(EMS_ABp,ba_vertical)[1]

        angle_value_3=angle(dc,ba_vertical)[1]
        
        if angle_value_2 <= 90:#down

            if angle_value > 90 and angle_value_3 > 90:
                cell_angle.append(-(180-angle_value))
            elif angle_value > 90 and angle_value_3 <= 90:
                cell_angle.append(180-angle_value)
            elif angle_value <= 90 and angle_value_3 <= 90:
    ...
    #cell_angle.to_csv(r"C:\Users\dell\Desktop\\cell_angle.txt", sep="\t")  
