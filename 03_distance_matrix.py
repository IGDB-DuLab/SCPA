...
"""
### input the folder of raw data of control or RNAi or RNAi_add
File_Directory = r'C:\Users\dell\Desktop\raw_data'  
...
### input the folder path which store all cell X, Y, Z coordinates of each embryo used to analyze the cell position correction in the simulated case 
...
###input the name of embryo length file
...
for each_emb in raw_data_list:
    ...
    for cell in Cell_list_714:
        ...
        if cell in emb_output_info.index:
            if type(emb_output_info.loc[cell]['time'])==pd.core.series.Series:
                time_point=int(emb_output_info.loc[cell]['time'].median()+0.5)-(emb_output_info.loc[cell]['time'].min())#int()+0.5
    ...
    #distance_exclude_nan = cell_pair_distance[~np.isnan(cell_pair_distance)]#drop(np.nan)
    #dist_max=distance_exclude_nan.max()#normalize embryo length
