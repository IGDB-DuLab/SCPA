...
### 1.input cell pair distance phenotype folder from “16_cell_pair_distance_phenotype.py”
cell_pair_distance_phenotype_path = os.path.abspath(r'C:\Users\dell\Desktop\output_folder\RNAi_different_matrix_AB256')

### 2.input relative cell position phenotype binary results from “09_position_phenotype.py”
RNAi_cellposition_qvalue_binary=pd.read_csv(r"C:\Users\dell\Desktop\output_folder\relative_cell_position_phenotype\RNAi_relative_cell_position_Q-value_binary_0.01.txt", index_col= 0,header=0,sep='\t')

### 3.output path and file name
Output_Folder = r'C:\Users\dell\Desktop\output_folder'   # assign the output folder path
output_file_name='cell module cell pair distance rate' #assign file name
#################################################################################################################
### 4.input the parametre files of "positionally_related_cells.txt" and "fate_clones.txt", which could be download from "data/Related files" folder.

location_cluster=pd.read_csv(r"C:\Users\dell\Desktop\revision data\code and table\data\Related files\positionally_related_cells.txt", index_col= 0,header=0,sep='\t')
location_cluster_list=list(set(location_cluster['Localization-cluster']))
location_cluster_list.sort()

location_fate_combine_cluster=pd.read_csv(r"C:\Users\dell\Desktop\revision data\code and table\data\Related files\fate_clones.txt", index_col= 0,header=0,sep='\t')
                    ...
                    if fate_cluster_cell_info is not None:
                        ...
                        if set(fate_cluster_cell_name)<=set(compare_matrix.index):
                            ...
                            cluster_compare_temp=pd.DataFrame([fat_cluster_name,abnormal_pair_1,cell_pair_1,abnormal_distance_fate_cluster,
                                                               ...
                                                               embryo_name,abnormal_cell])
                            cluster_compare_temp=cluster_compare_temp.T
                            cluster_compare_temp.columns=['FC class','acd number of FC','total number of FC','Facd within FC',
                                                          'PC class','acd number of PC','total number of PC','Facd within PC',
                                                          'acd number of unrelated cells','total number of unrelated cells','Facd within unrelated cells',
                                                          'KD embryo','Cell with position defect']  
                            
                            cluster_compare_temp=cluster_compare_temp.set_index(['KD embryo'])
...
gene_cluster_cell_compare.to_csv(os.path.join(Output_Folder,output_file_name+".txt"), sep="\t")  
 
##############
gene_cluster_position_cell_compare=gene_cluster_cell_compare
clusters=list(set(gene_cluster_position_cell_compare['FC class']))
clusters.sort()
...
for i in clusters:
    #i='Caaa'
    temp_value=gene_cluster_position_cell_compare[gene_cluster_position_cell_compare['FC class']==i]
    actrual=temp_value['Facd within FC']
    other_location=temp_value['Facd within PC']
    other=temp_value['Facd within unrelated cells']#####
    
    df1=len(actrual)-1
    df2=len(other_location)-1    
    df3=len(other)-1
    actural_N.append(len(actrual))
    other_location_N.append(len(other_location))
    other_N.append(len(other))
    
    ci_1 = stats.t.interval(0.95, df1, loc=0, scale=stats.sem(actrual.dropna()))[1]
    ci_2 = stats.t.interval(0.95, df2, loc=0, scale=stats.sem(other_location.dropna()))[1]
    ci_3 = stats.t.interval(0.95, df3, loc=0, scale=stats.sem(other.dropna()))[1]
    
    actrual_mean=actrual.mean()
    other_location_mean=other_location.mean()    
    other_mean=other.mean()
    
    pvalue_12=stats.wilcoxon(list(actrual), list(other_location), alternative='less')[1]
    pvalue_13=stats.wilcoxon(list(actrual), list(other), alternative='less')[1]
    pvalue_23=stats.wilcoxon(list(other_location), list(other), alternative='less')[1]

    cluster_compare=[actrual_mean,other_location_mean,other_mean,ci_1,ci_2,ci_3,pvalue_12,pvalue_13,pvalue_23]
    cluster_compares.append(cluster_compare)
cluster_compares=pd.DataFrame(cluster_compares)    
cluster_compares.index=clusters
cluster_compares.columns=['Facd_FC_mean','Facd_PC_mean','Facd_unrelated_cells_mean','FC_95CI','PC_95CI','unrelated_cell_95CI',
                          'wilcoxon p-value FC VS PC','wilcoxon p-value FC VS unrelated_cell','wilcoxon p-value PC VS unrelated_cell']
cluster_compares.to_csv(os.path.join(Output_Folder,output_file_name+"_statistics.txt"), sep="\t")  
