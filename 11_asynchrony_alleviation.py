...
### 1.input the cell cycle length and division time data of control and RNAi from "01_proliferation.py"
...
### 2.input the proliferation rate file from "05_proliferation_rate.py"
RNAi_slope = pd.read_csv(r'C:\Users\dell\Desktop\output_folder/RNAi_slope.txt', index_col=0, sep='\t')

### 3.input the follow files obtain from '06_asynchrony_phenotype.py'
...
division_asynchrony_phenotype=division_asynchrony_phenotype[selected_embryos].dropna(axis=0,how='all')


total_cell_list=list(division_asynchrony_phenotype.index)
total_cell_list=cell_name_transfer(total_cell_list)

division_asynchrony_phenotype.index=total_cell_list
...
division_asynchrony_phenotype.index=cell_name_transfer(list(division_asynchrony_phenotype.index))
...
order_info=order_info[order_info['q-value binary']==0]
...
for i in range(len(order_info.index)):
    ...
    selected_control=division_asynchrony_phenotype.loc[mother_cell]
    selected_control_embryo=list(selected_control[selected_control==0].index)
    random_embryo=random.sample(selected_control_embryo,1)
    

    children_cell=temp['children cell']
    embryo_children_cell.append(embryo+'+'+random_embryo[0]+'+'+mother_cell+'+'+children_cell)
    cell_list_temp=stage_cell_list(children_cell)
    division_vector=Celldivition_RNAi_matrix.loc[cell_list_temp][random_embryo]

    emb_division=Celldivition_RNAi_matrix.loc[children_cell][random_embryo]
    emb_cycle=Cellcycle_RNAi_matrix.loc[children_cell][random_embryo]

    #Cellcycle_CTR_temp=Cellcycle_CTR.drop([embryo],axis=1)
...
for i in abnormal_mother_children_rmsd_simulation.columns:
    ...
    actrual_rmsd=RNAi_division_asynchrony_RMSD[embryo_name_actural].loc[children_name]
    ...
    simulation_rmsd=RNAi_division_asynchrony_RMSD[embryo_name_simulate].loc[children_name]
    ...
    get_values=[actrual_rmsd,simulation_rmsd,actrual_zscore,actrual_zscore_control]
    rmsd_compare.append(get_values)

rmsd_compare=pd.DataFrame(rmsd_compare)
rmsd_compare.columns=['Observed deviation from normal embryos (RMSD)','Expected deviation from normal embryos (RMSD)','actrual_zscore','actrual_zscore_control']
...
division_compensation['Fraction of cells in the daughter cell generation with division asynchrony defects'] = abnormal_rate_all
...
division_asynchrony_compensation_withqvalue_merge['q-value'] =   mother_qvalue_all
#division_asynchrony_compensation_withqvalue_merge['children_qvalue']  =  children_qvalue_all

division_asynchrony_compensation_withqvalue_merge['Consistent with correction/alleviation actrual'] =   compensation_actrual
division_asynchrony_compensation_withqvalue_merge['Consistent with correction/alleviation simulated']  =  compensation_control

division_asynchrony_compensation_withqvalue_merge.columns=['Observed deviation from normal embryos (RMSD)',
       'Expected deviation from normal embryos (RMSD)', 'zscore between observed and expected',
       'zscore between simulated and expected', 'Daugther cell', 'Cell with division asynchrony defect',
       'Fraction of cells in the daughter cell generation with division asynchrony defects',
       'q-value', 'Consistent with correction/alleviation actrual',
       'Consistent with correction/alleviation simulated']
