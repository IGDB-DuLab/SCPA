...
### 1.input the follow files obtain from “01_proliferation.py”
Cellcycle_CTR_matrix = pd.read_csv(r"C:\Users\dell\Desktop\output_folder\Cell_cycle_length_matrix_ctr.txt", index_col= 0,header=0,sep='\t')
Celldivision_CTR_matrix = pd.read_csv(r"C:\Users\dell\Desktop\output_folder\Cell_division_timing_matrix_ctr.txt", index_col= 0,header=0,sep='\t')
Celldivision_RNAi_matrix = pd.read_csv(r"C:\Users\dell\Desktop\output_folder\Cell_division_timing_matrix_RNAi.txt", index_col= 0,header=0,sep='\t')  

### 2.input the follow files obtain from “02_expression.py”
CTR_exp_matrix714_binary= pd.read_csv(r"C:\Users\dell\Desktop\output_folder\CTR_CND-1_expression_value_matrix_binary.txt", index_col= 0,header=0,sep='\t')
RNAi_exp_matrix714_binary= pd.read_csv(r"C:\Users\dell\Desktop\output_folder\RNAi_CND-1_expression_value_matrix_binary.txt", index_col= 0,header=0,sep='\t')

### 3.input the follow files obtain from “14_expression_onset.py”
lineage_onset_CTR = pd.read_csv(r"C:\Users\dell\Desktop\output_folder\Control_CND-1_expression_onset_matrix.txt", index_col= 0,header=0,sep='\t')
lineage_onset_RNAi = pd.read_csv(r"C:\Users\dell\Desktop\output_folder\RNAi_CND-1_expression_onset_matrix.txt", index_col= 0,header=0,sep='\t')

### 4.input the divsion timing phenotype, you can obtain the data by using “10_other_phenotype.py” code to analysis the division time matrix of control and RNAi.
single_cell_divisiontime_phenotype= pd.read_csv(r"C:\Users\dell\Desktop\output_folder\cell division phenotype\RNAi cell division time_Q-value_binary_0.01.txt", index_col= 0,header=0,sep='\t')

### 5.output path and file name
Output_Folder = r'C:\Users\dell\Desktop\output_folder'   # assign the output folder path
output_file_name='CND-1_expression_and_division_coordination' #input file name
################################functions
...
emb_info_unc['track']=list(emb_info_unc.index)    
emb_info_unc=emb_info_unc.set_index(['embryo'])
emb_info_unc.columns=['coordination_status','division_time_phenotype','expression_status','cell_track']    
...
for i in Cell_exp_18track.columns:
    ...
    embryos_categrays_sub=embryos_categrays[embryos_categrays['cell_track']==i]
    ...
    for j in embryos_categrays_sub.index:    
        ...
        onset_delay_rate=RNAi_expression_time_temp/WT_expression_time
       
        division_onset_time.append([WT_division_time,RNAi_division_time,delay_rate,WT_expression_time,RNAi_expression_time_temp,onset_delay_rate])
    
        
    division_onset_time=pd.DataFrame(division_onset_time)
    division_onset_time.index=embryos_categrays_sub.index
    division_onset_time.columns=['ctr_division_time','RNAi_division_division_time','division_time_delay_rate','ctr_expression_onset','RNAi_expression_onset','onset_delay_rate']
